...
function wpcc_header() {
	...
	echo "\n" . '<!-- WP Chinese Conversion Plugin Version 1.1.14.1 -->';
...
function wpcc_output_navi($args = '') {
	...
	$output .= '	<span id="wpcc_original_link" class="' . ( $wpcc_target_lang == false ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a href="' . esc_url($default_url) . '" title="' . esc_html($noconverttip) . '">' . esc_html($noconverttip) . '</a></span>' . "\n";

	foreach($wpcc_langs_urls as $key => $value) {
		$tip = !empty($wpcc_options[$wpcc_langs[$key][1]]) ? esc_html($wpcc_options[$wpcc_langs[$key][1]]) : $wpcc_langs[$key][2];
		$output .= '	<span id="wpcc_'. $key . '_link" class="'. ( $wpcc_target_lang == $key ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a rel="nofollow" href="'. esc_url($value) .'" title="' . esc_html($tip) . '" >' . esc_html($tip) . '</a></span>' . "\n";
	}
...
function wpcc_output_navi2() {
	...
	$output = "\n" . '<div id="wpcc_widget_inner"><!--WPCC_NC_START-->' . "\n";
	$output .= '	<span id="wpcc_original_link" class="' . ( $wpcc_target_lang == false ? 'wpcc_current_lang' : 'wpcc_lang' ) .'" ><a href="' . esc_url($default_url) . '" title="不轉換">不轉換</a></span>' ."\n";
	$output .= '	<span id="wpcc_cn_link" class="'. ( $wpcc_target_lang == 'zh-cn' ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a rel="nofollow" href="'. esc_url($wpcc_langs_urls['zh-cn']) . '" title="大陆简体" >大陆简体</a></span>' ."\n";
	$output .= '	<span id="wpcc_tw_link" class="'. ( $wpcc_target_lang == 'zh-tw' ? 'wpcc_current_lang' : 'wpcc_lang' ) .'"><a rel="nofollow" href="'. esc_url($wpcc_langs_urls['zh-tw']) .'" title="台灣正體" >台灣正體</a></span>' ."\n";
	...
	<span id="wpcc_more_links_inner_more" class="'. ( ( $wpcc_target_lang == false || $wpcc_target_lang == 'zh-cn' || $wpcc_target_lang == 'zh-tw' ) ? 'wpcc_lang' : 'wpcc_current_lang' ) . '"><a href="#" onclick="return false;" >其它中文</a></span>
		<span id="wpcc_more_links_inner" >
			<span id="wpcc_hans_link" class="' . ( $wpcc_target_lang == 'zh-hans' ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a rel="nofollow" href="' . esc_url($wpcc_langs_urls['zh-hans']) . '" title="简体中文" >简体中文' . '</a></span>
			<span id="wpcc_hant_link" class="' . ( $wpcc_target_lang == 'zh-hant' ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a rel="nofollow" href="' . esc_url($wpcc_langs_urls['zh-hant']) . '" title="繁體中文" >繁體中文' . '</a></span>
			<span id="wpcc_hk_link" class="' . ( $wpcc_target_lang == 'zh-hk' ? 'wpcc_current_lang' : 'wpcc_lang' ) . '"><a rel="nofollow" href="' . esc_url($wpcc_langs_urls['zh-hk']) . '" title="港澳繁體" >港澳繁體</a></span>
			<span id="wpcc_sg_link" class="' . ( $wpcc_target_lang == 'zh-sg' ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a rel="nofollow" href="' . esc_url($wpcc_langs_urls['zh-sg']) . '" title="马新简体" >马新简体</a></span>
		</span>
	</span>';
...
function wpcc_ob_callback($buffer) {
	...
	if( $wpcc_target_lang && !$wpcc_direct_conversion_flag ) {
		$wpcc_home_url = wpcc_link_conversion(home_url('/'));
		if( !empty($wpcc_options['wpcc_use_permalink']) )
			$wpcc_home_url = trailingslashit($wpcc_home_url);
		$buffer = preg_replace('|(<a\s[^<>]*?href=([\'"]))' . preg_quote(esc_url(home_url('/')), '|') . '(\2[^<>]*?>)|', '\\1' . esc_url($wpcc_home_url) . '\\3', $buffer);
	}
